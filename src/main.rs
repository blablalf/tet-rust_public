...
impl AppState {
        ...
        self.gl.draw(args.viewport(), |c, gl| {
            ...
            for (line_index, line) in self.grid.iter().enumerate() {
                ...
                for (case_index, case) in line.iter().enumerate() {
                    if *case != 0 {
                        rectangle(
                            get_number_color(*case),
                            square,
                            c.transform.trans(
                                ((case_index * SQUARE_SIZE as usize) + 1) as f64,
                                ((line_index * SQUARE_SIZE as usize) + 1) as f64,
                            ),
                            gl,
                        ); // TO-DO -> put the correct color instead of RED
                    }
                }
            }
                ...
                for (case_index, case) in line.iter().enumerate() {
                    if *case != 0 {
                        rectangle(
                            ...
                            square,
                            c.transform.trans(
                                pos_x + (case_index as u32 * SQUARE_SIZE) as f64,
                                pos_y + (line_index as u32 * SQUARE_SIZE) as f64,
                            ),
                            gl,
                        ); // transform to enlarge our piece shape
                    }
                }
            }
        })
    ...
    fn update(&mut self, _args: &UpdateArgs) {
        ...
        if !self.current_piece.auto_set_placed(
            self.grid,
            self.current_piece.pos_y,
            self.piece_speed,
        ) {
        ...
        for (line_index, _line) in self.grid.clone().iter().enumerate() {
            while self.is_complete_line(line_index as u32) {
                for line_index_temp in 0..line_index {
                    self.grid[line_index - line_index_temp] =
                        self.grid[line_index - line_index_temp - 1];
                }
        ...
        if let &Button::Keyboard(key) = args {
            match key {
                ...
                Key::Left => {
                    ...
                    if !self.current_piece.is_colliding(
                        self.grid,
                        self.current_piece.matrix,
...
fn main() {
    ...
    let mut window: GlutinWindow =
        WindowSettings::new(game_name, [PIXEL_GRID_WIDTH, PIXEL_GRID_HEIGTH])
            .exit_on_esc(true)
            .build()
            .unwrap();
